...
- **Derive State During Render (React Compiler Strictness):** NEVER use `useEffect` to synchronize local state with incoming props or external stores (e.g., `useEffect(() => setLocalValue(value), [value])`). This is a strict anti-pattern flagged by React Compiler because calling `setState` synchronously within a `useEffect` triggers cascading renders. Instead, store the previous value in state and update the local value directly during render: `const [prev, setPrev] = useState(value); if (value !== prev) { setPrev(value); setLocalValue(value); }`. Or initialize state dynamically using `key={value}` on the component wrapper if full remounting is acceptable.
