...
- **Settings Layout & Hierarchy:** Group related settings into dedicated cards (e.g., using `SettingsCard`). For settings with descriptions, strictly prefer vertical lists with dividers (`divide-y`) over multi-column grids to prevent visual crowding. Enforce a clear hierarchy: Title (left) > Description (left) > Control (right). Avoid using standard HTML `<select>` inputs for static configuration choices (like themes or layout modes). Instead, prefer custom interactive segmented controls (grid layouts) using `animate-in` and `shadow-engraved` to indicate the active state. For settings that trigger a simple reset or restart action, use the unified `ResetButton` component (an icon button) placed in the same row as the Title, with the Description spanning full width underneath. Large settings sections must be code-split into isolated, single-responsibility components within a dedicated feature folder (e.g., `features/settings/components/sound-fx/`). When doing so, use `index.tsx` as the primary entry point (e.g., `appearance/index.tsx`) rather than a redundant filename (like `appearance-section.tsx`) to clearly identify the parent component.
...
- **Strict TypeScript & Linting:** The project strictly enforces `typescript/no-explicit-any` and bans `@ts-ignore` bypasses via `oxlint`. Additionally, deep relative imports (`../../`) are actively blocked by a custom `no-restricted-imports` oxlint rule. Code formatting is handled by `oxfmt`. The production build is configured to strictly fail on any warnings (`--deny-warnings`). NEVER use `any`. Use `unknown` and type guard it if necessary. When using Vite, `verbatimModuleSyntax` is enabled, so you must use type-only imports for TypeScript types (e.g., `import { type ReactNode, type ComponentProps } from "react";`).
...
- **Import Aliases (Strict Rule):** NEVER use deep relative imports (e.g., `../../` or `../../../`). You MUST always use the `@` alias for absolute imports when importing from outside the current feature module or from global directories. Relative imports (e.g., `./` or `../`) are ONLY permitted for co-located files within the exact same feature boundary to preserve cohesion. Furthermore, when importing from directories that have an `index.ts` barrel file (e.g., `src/components/icons`, `src/stores`, `src/hooks`, `src/schemas`), always import through the barrel root (e.g., `import { FolderLinkIcon } from "@/components/icons";`) rather than directly importing the individual files.
...
- **PWA Installation Events:** The `beforeinstallprompt` and `appinstalled` events fire outside the typical React lifecycle. Do NOT attempt to capture them exclusively inside a React `useEffect`. Always stash `beforeinstallprompt` via a lightweight inline script in `index.html` (e.g., `window.__deferredPrompt`), and specify that `appinstalled` clears `window.__deferredPrompt` and `window.__isInstallable` before dispatching a custom global event (like `app-installable`) that React can reliably consume once it mounts to synchronize its internal state.
