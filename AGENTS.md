...
- **Notification Actions & Undo:** When implementing undo actions via notifications, utilize `successLabel` (e.g., `successLabel: "Link restored"`) instead of manually dismissing the toast. This allows the notification to morph smoothly and provides better contextual feedback without abruptly disappearing.
