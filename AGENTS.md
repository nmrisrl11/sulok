...
- **Strict TypeScript & Build:** The project strictly enforces `@typescript-eslint/no-explicit-any` and bans `@ts-ignore` bypasses via `oxlint`. The production build is configured to strictly fail on any warnings (`--deny-warnings`). NEVER use `any`. Use `unknown` and type guard it if necessary. When using Vite, `verbatimModuleSyntax` is enabled, so you must use type-only imports (e.g., `import type { ... }`).
- **Fast Refresh Strictness:** To ensure React Fast Refresh works perfectly without losing component state, component files (`.tsx`) must strictly ONLY export components. Extract non-components (like `cva` variants, custom hooks, or Context objects) into dedicated files (e.g. `button-variants.ts` or `hooks/use-theme.ts`). Do not bypass the `react/only-export-components` rule.
