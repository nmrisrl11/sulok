...
- **PWA Installation Events:** The `beforeinstallprompt` and `appinstalled` events fire outside the typical React lifecycle. Do NOT attempt to capture them exclusively inside a React `useEffect`. Always capture and stash these events via a lightweight inline script in `index.html` (e.g., `window.__deferredPrompt`), and dispatch a custom global event (like `app-installable`) that React can reliably consume once it mounts to synchronize its internal state.
