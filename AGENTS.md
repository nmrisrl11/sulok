...
- **PWA Installation Events:** The `beforeinstallprompt` event fires very early during page load. Do NOT attempt to capture it exclusively inside a React `useEffect` as React often mounts too late. Always capture and stash the event via a lightweight inline script in `index.html` (e.g., `window.__deferredPrompt`), or dispatch a custom global event that React can reliably consume once it mounts.
